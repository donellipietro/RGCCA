...
auto fit_model(Triangulation<1,1> I_D, Triangulation<1,1> I_T,
  ...
  options.allow_reconstruction_constraint_compensation = false;
  ...
  sparse_matrix_t Psi_grid_D = internals::point_basis_eval(Vh, grid_D);
  FeSpace Vh_T(I_T, P1<1>);
  sparse_matrix_t Psi_grid_T = internals::point_basis_eval(Vh_T, grid_T);
...
int main(int argc, char* argv[]) {
  ...
  // Fit the model
  fit_model(I_D, I_T, n_obs, n_comp, sd_noise, grid_D, grid_T, path_data, path_results);
