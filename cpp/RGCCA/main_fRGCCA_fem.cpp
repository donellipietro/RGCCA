...
std::vector<std::vector<double>>
  parse_lambda_grid_weights(const nlohmann::json &options) {
...
int main(int argc, char *argv[]) {
  ...
  std::string path_mesh =
    "../../" + jroot["path_list"].value("mesh", "./mesh/");
  std::string path_data =
    "../../" + jroot["path_list"].value("data", "./data/");
  ...
  bool lambda_selection_weights =
    jroot["options"].value("lambda_selection_weights", false);
    ...
    for (std::size_t h = 0; h < boot_results.size(); ++h) {
      ...
      for (std::size_t i = 0; i < boot.lambda_grid.size(); ++i) {
        ...
        for (std::size_t j = 0; j < boot.w_fit_by_lambda[i].size(); ++j) {
          ...
          // fdaPDE bootstrap confidence intervals, evaluated on the grid
          auto [w_ci_low_grid, w_ci_high_grid] = rgcca.bootstrap_weights_ci(h, i, j, Psi_grid);
          write_csv(path_results + "bootstrap_weights_ci_comp" +
            std::to_string(h + 1) + "_lambda" +
            std::to_string(i + 1) + "_block" +
            std::to_string(j + 1) + "_grid.csv",
            make_ci_matrix(w_ci_low_grid, w_ci_high_grid),
            std::vector<std::string>{"lower", "upper"});
        }
      }
    }
