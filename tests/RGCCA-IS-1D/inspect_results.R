# = ========================================================================== =
# - Test: RGCCA - Naive - Inspect results
...
#' Source an R file only when it is present.
#'
#' @param path File or directory path.
#' @return The value produced by `source_if_exists`.
...
## Plot
n_models <- length(loaded_qlt_results$model_names)
for (g in 1:4) {
  pdf(file = paste(path_list$images, "block", g, "_qualitative.pdf", sep = ""), width = 15, height = 2.5*3)
  plot_qualitative_results(g, loaded_qnt_results, loaded_qlt_results)
  dev.off()
}
