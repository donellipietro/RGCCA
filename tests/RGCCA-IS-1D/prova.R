...
data <- generate_data(options, seed = 0)
...
results1full <- R_RGCCA("R_GCCA_cov", data, test_options)
results2full <- CPP_RGCCA("CPP_RGCCA", data, test_options, path_list)

# Fitto il modello con una sola componente
test_options$model_options$n_comp <- 1
results1 <- R_RGCCA("R_GCCA_cov", data, test_options)
results2 <- CPP_RGCCA("CPP_RGCCA", data, test_options, path_list)

sum(abs(results1$results$A_hat_locs[[1]][,1] - results2$results$A_hat_locs[[1]][,1]))
sum(abs(results1$results$A_hat_locs[[2]][,1] - results2$results$A_hat_locs[[2]][,1]))
sum(abs(results1$results$A_hat_locs[[3]][,1] - results2$results$A_hat_locs[[3]][,1]))
sum(abs(results1$results$A_hat_locs[[4]][,1] - results2$results$A_hat_locs[[4]][,1]))
...
# Faccio la Deflation a mano
data1 <- data
for(i in 1:4) {
  R <- diag(1, data$dimensions$n, data$dimensions$n)
  norm <- sum(results1$results$H_hat[[i]][,1]^2)
  R <- R - results1$results$H_hat[[i]][,1] %*% t(results1$results$H_hat[[i]][,1])/norm
  data1$X[[i]] <- R %*% data$X[[i]]
}

# Fitto il modello con una sola componente sui dati dopo la deflation
results1 <- R_RGCCA("R_GCCA_cov", data1, test_options)
results2 <- CPP_RGCCA("CPP_RGCCA", data1, test_options, path_list)

# Ora le stime coincidono
sum(abs(results1$results$A_hat_locs[[1]][,1] - results2$results$A_hat_locs[[1]][,1]))
sum(abs(results1$results$A_hat_locs[[2]][,1] - results2$results$A_hat_locs[[2]][,1]))
sum(abs(results1$results$A_hat_locs[[3]][,1] - results2$results$A_hat_locs[[3]][,1]))
sum(abs(results1$results$A_hat_locs[[4]][,1] - results2$results$A_hat_locs[[4]][,1]))

sum(abs(results1$results$A_hat_locs[[1]][,1] - results1full$results$A_hat_locs[[1]][,2]))
sum(abs(results1$results$A_hat_locs[[2]][,1] - results1full$results$A_hat_locs[[2]][,2]))
sum(abs(results1$results$A_hat_locs[[3]][,1] - results1full$results$A_hat_locs[[3]][,2]))
sum(abs(results1$results$A_hat_locs[[4]][,1] - results1full$results$A_hat_locs[[4]][,2]))

sum(abs(results2$results$A_hat_locs[[1]][,1] - results2full$results$A_hat_locs[[1]][,2]))
sum(abs(results2$results$A_hat_locs[[2]][,1] - results2full$results$A_hat_locs[[2]][,2]))
sum(abs(results2$results$A_hat_locs[[3]][,1] - results2full$results$A_hat_locs[[3]][,2]))
sum(abs(results2$results$A_hat_locs[[4]][,1] - results2full$results$A_hat_locs[[4]][,2]))
...
plot_list <- list()
for(i in 1:4) {
  plot_list[[i]] <- plot.curve(data$locations_D, results2full$results$A_hat_locs[[i]][,2],
                               true = data$A_locs[[i]][,2]) + std_plot_settings_curves()
}
plot <- arrangeGrob(grobs = plot_list)
grid.arrange(plot)
...
test_options$model_options$n_comp <- 3
results1 <- R_RGCCA("R_RGCCA", data, test_options)
results2 <- CPP_RGCCA("CPP_RGCCA", data, test_options, path_list)
...
sum(abs(results1$results$H_hat[[1]][,1] - results2$results$H_hat[[1]][,1]))
sum(abs(results1$results$H_hat[[2]][,1] - results2$results$H_hat[[2]][,1]))
sum(abs(results1$results$H_hat[[3]][,1] - results2$results$H_hat[[3]][,1]))
sum(abs(results1$results$H_hat[[4]][,1] - results2$results$H_hat[[4]][,1]))

sum(abs(results1$results$A_hat_locs[[1]][,2] - results2$results$A_hat_locs[[1]][,2]))
sum(abs(results1$results$A_hat_locs[[2]][,2] - results2$results$A_hat_locs[[2]][,2]))
sum(abs(results1$results$A_hat_locs[[3]][,2] - results2$results$A_hat_locs[[3]][,2]))
sum(abs(results1$results$A_hat_locs[[4]][,2] - results2$results$A_hat_locs[[4]][,2]))

sum(abs(results1$results$H_hat[[1]][,2] - results2$results$H_hat[[1]][,2]))
sum(abs(results1$results$H_hat[[2]][,2] - results2$results$H_hat[[2]][,2]))
sum(abs(results1$results$H_hat[[3]][,2] - results2$results$H_hat[[3]][,2]))
sum(abs(results1$results$H_hat[[4]][,2] - results2$results$H_hat[[4]][,2]))

sum(abs(results1$results$A_hat_locs[[1]][,3] - results2$results$A_hat_locs[[1]][,3]))
sum(abs(results1$results$A_hat_locs[[2]][,3] - results2$results$A_hat_locs[[2]][,3]))
sum(abs(results1$results$A_hat_locs[[3]][,3] - results2$results$A_hat_locs[[3]][,3]))
sum(abs(results1$results$A_hat_locs[[4]][,3] - results2$results$A_hat_locs[[4]][,3]))

sum(abs(results1$results$H_hat[[1]][,3] - results2$results$H_hat[[1]][,3]))
sum(abs(results1$results$H_hat[[2]][,3] - results2$results$H_hat[[2]][,3]))
sum(abs(results1$results$H_hat[[3]][,3] - results2$results$H_hat[[3]][,3]))
sum(abs(results1$results$H_hat[[4]][,3] - results2$results$H_hat[[4]][,3]))
