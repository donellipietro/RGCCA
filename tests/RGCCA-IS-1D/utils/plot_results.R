...
plot_qualitative_results <- function(group, quantitative_results, qualitative_results) {
  ...
  for (m in seq_along(model_names)) { # m <- 4
    ...
    plots_locs <- labeled_plots_grid(arrangeGrob(grobs = plot_list, ncol = 4), paste("A - model:", label_model), labels_cols, labels_rows)
  ...
  for (m in seq_along(model_names)) { # m <- 4
    ...
    ## Arrange labeled grids for each display mode
    plots_locs <- labeled_plots_grid(arrangeGrob(grobs = plot_list, ncol = 4), paste("Astar - model:", label_model), labels_cols, labels_rows)
    # plots_grid <- labeled_plots_grid(arrangeGrob(grobs = plot_list, ncol = 4), "A at HR grid", labels_cols, labels_rows)
