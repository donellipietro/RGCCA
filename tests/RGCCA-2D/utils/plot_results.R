...
#' Plot quantitative summaries for loaded results.
#'
#' @param loaded_results Loaded quantitative or qualitative result structure.
#' @return The value produced by `plot_quantitative_results`.
...
#' Plot representative fitted functions against truth.
#'
#' @param group Optional group labels.
#' @param quantitative_results Loaded quantitative results used to rank representative batches.
#' @param qualitative_results Loaded fitted functions and reference data for plotting.
#' @return The value produced by `plot_qualitative_results`.
plot_qualitative_results <- function(group, quantitative_results, qualitative_results) {
  ...
  for (m in seq_along(model_names)) { # m <- 1
    ...
    plots_locs <- labeled_plots_grid(arrangeGrob(grobs = plot_list, ncol = 4), paste("A at location - model:", label_model), labels_cols, labels_rows)
    # plots_grid <- labeled_plots_grid(arrangeGrob(grobs = plot_list, ncol = 4), "A at HR grid", labels_cols, labels_rows)
    ...
    ## Arrange labeled grids for each display mode
    plots <- labeled_plots_grid(arrangeGrob(grobs = plot_list, ncol = 4), paste("E at location - model:", label_model), labels_cols, labels_rows)
