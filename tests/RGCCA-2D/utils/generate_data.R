#' Load a saved spatial region object by name.
#'
#' @param region_name Saved region identifier.
#' @return The value produced by `load_region_object`.
...
#' Evaluate saved region loading coefficients at given spatial locations.
#'
#' @param region_obj Saved region object.
#' @param locs_mat Location matrix.
#' @return The value produced by `eval_region_loadings`.
eval_region_loadings <- function(region_obj, locs_mat) {
  ...
  as.matrix(Phi %*% region_obj$coef_mat)
}
...
generate_data <- function(test_options, seed = 0) {
  ...
  ## Helpers ----
  #' Evaluate a sine component at the requested frequency.
  #'
  #' @param t Time points or time duration.
  #' @param f_hz Frequency in Hz.
  #' @return The value produced by `e_gen_hz`.
  ...
  if (is.null(TR)) {
    ...
    if (is.null(n_times)) {
      stop("Both TR and n_times are NULL.", call. = FALSE)
    } else {
      TR <- T_sec / (n_times - 1)
    }
  }
  ...
  for (g in 1:n_groups) { # g = 1
    ...
    n_locs_D_target <- test_options$dimensions$n_locs_mult[g] * n_locs_D  # scalar target
    set.seed(0)
    ...
    for (h in 1:n_comp) {
      max_a <- max(A_grid[[g]][,h])
      max_a <- if (is.na(max_a) || max_a == 0) 1 else max_a
      A_grid[[g]] <- A_grid[[g]]/max_a
      A_locs[[g]] <- A_locs[[g]]/max_a
    }
    ...
    X_locs[[g]] <- E_locs[[g]] %*% t(A_locs[[g]])   # n_T x n_locs_Dg
  ...
  for (g in 1:n_groups) {
    ...
    X[[g]] <- as.matrix(X_locs[[g]] + noise[[g]])
