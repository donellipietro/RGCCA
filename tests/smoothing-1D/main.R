...
domain <- generate_domain(
  ...
  test_options$dimensions$n_knots
)

## Sample the locations
locs_eq_knots <- test_options$domain_and_locations$locs_eq_knots
n_locs <- test_options$dimensions$n_locs
locations <- generate_locations(domain, locs_eq_knots, n_locs)

## Grid
n_knots_grid <- test_options$dimensions$n_knots_grid
grid <- generate_locations(domain, FALSE, n_knots_grid, type = "regular")
...
if (RUN$tests) {
  for (batch_idx in 1:test_options$test_options$n_reps) {
    ...
    if (any(!file.exists(file_model_vect)) || FORCE_FIT || FORCE_EVALUATE) {
      data <- generate_data(
        domain = domain, locations = locations, grid = grid,
